...
class ResultsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        for i in 0...requirmentsArr.count - 1 {
            labelsMatrixView1.addRecord(record: [String(i + 1), requirmentsArr[i], String(format: "%.2f", Requirments.shared().reqArrayFinal[i])])
        }
        ...
        for i in 0...RationArr.count - 1 {
            labelsMatrixView2.addRecord(record: [String(i + 1), RationArr[i],String(format: "%.2f", Requirments.shared().rationArrayFinal[i])])
        }
        ...
        for i in 0...WaterRationArr.count - 1 {
            labelsMatrixView3.addRecord(record: [String(i + 1), WaterRationArr[i], String(Requirments.shared().waterArrayFinal[i])])
        }
        ...
        for i in 0...PremixArr.count - 1 {
            labelsMatrixView4.addRecord(record: [String(i + 1), PremixArr[i], String(format: "%.2f",Requirments.shared().primexArrayFinal[i])])
        }
